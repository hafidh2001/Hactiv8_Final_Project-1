...
export const storeReflection = async (owner_id, success, low_point, take_away) => {
    try{
...
export const editReflection = async (owner_id, reflection_id, success, low_point, take_away) => {
    try{
        if(success == null || low_point == null || take_away == null)
            return {
                status: "error",
            ...
            const data = await db.query(
                ...
                [owner_id, reflection_id, success, low_point, take_away, date]
            );
            const res = data?.rows[0];
            return { status: "success", res, user };
    }catch (error) {
